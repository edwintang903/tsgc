# Contributing to tsgc

This outlines how to propose a change to `tsgc`. For more detailed info
about contributing to this and other tidyverse-style packages, please see
the [development contributing
...
changes are made in the _source_ file. This generally means you'll need to
edit [roxygen2 comments](https://roxygen2.r-lib.org/articles/roxygen2.html)
...
* Fork the package and clone onto your computer. If you haven't done this
  before, we recommend using `usethis::create_from_github("edwintang903/tsgc", fork = TRUE)`.
...
* For user-facing changes, add a bullet to the top of `NEWS.md` (i.e.
  above the current development version header) describing the changes
  made, followed by your GitHub username and, if applicable, the issue
  number in parentheses (e.g. `list_files() now ignores hidden files.
  @jimhester (#219)`).
...
Please note that the `tsgc` project is released with a [Contributor Code

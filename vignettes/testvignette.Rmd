...
A small helper wraps the conversion for the common case of a daily
series with no gaps (every calendar day present, i.e. `pattern = 1`):
...
conv <- xts_to_idx(england_weather_2021[, 1:4], start.pos = england_idx$start)
...
estimation.pos.start <- idx_to_pos <- function(cal, date) {
...
(setting `xpred.new`/`xpred_lead.new`/`xpred_targ.new`); the fields are
set directly on the `FilterResultsLI` object instead.

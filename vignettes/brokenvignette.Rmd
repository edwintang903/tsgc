...
"quarters"` and an anchor built from the first `yearqtr` in the series.
`amount = 1` since every quarter is present (`pattern = 1`).
...
nintendo_cal <- idx_calendar(
  anchor = zoo::index(y_q_xts)[1],
  anchor_pos = 1L,
  amount = 1, unit = "quarters",
  posixct = FALSE
)
# position <-> yearqtr lookup, since idx_to_date() does plain numeric
# arithmetic and does not know about zoo::yearqtr arithmetic directly.
nintendo_pos_to_yq <- function(pos) {
  zoo::as.yearqtr(nintendo_cal$anchor) + (pos - nintendo_cal$anchor_pos) / 4
}
nintendo_yq_to_pos <- function(yq) {
  as.integer(round(nintendo_cal$anchor_pos +
                     (zoo::as.yearqtr(yq) - zoo::as.yearqtr(nintendo_cal$anchor)) * 4))
}
```
...
etrading_cal <- idx_calendar(
  anchor = zoo::index(y_m_xts)[1],
  anchor_pos = 1L,
  amount = 1, unit = "months",
  posixct = FALSE
)
etrading_pos_to_ym <- function(pos) {
  zoo::as.yearmon(etrading_cal$anchor) + (pos - etrading_cal$anchor_pos) / 12
}
etrading_ym_to_pos <- function(ym) {
  as.integer(round(etrading_cal$anchor_pos +
                     (zoo::as.yearmon(ym) - zoo::as.yearmon(etrading_cal$anchor)) * 12))
}
```
...
print(idx_values(get_timeframe(res.eng.x$xpred_lead.new, est.end.eng+1, est.end.eng+7)))
print(idx_values(get_timeframe(idx_lag(res.eng.x$xpred_lead.new, 4), est.end.eng+1, est.end.eng+7)))
```
...
Quarterly positions were converted using `nintendo_yq_to_pos()` defined
in the data-conversion section above.

```{r freq-quarterly}
n.lag.q <- nintendo_yq_to_pos("2017 Q1") - nintendo_yq_to_pos("2006 Q4")

mod.q <- SSModelLeadingIndicator$new(
  Y = nintendo_idx,
  sea.period = 4,
  n.lag = n.lag.q,
  start = nintendo_yq_to_pos("2017 Q1"),
  end = nintendo_yq_to_pos("2019 Q4"),
...
```{r freq-monthly}
n.lag.m <- etrading_ym_to_pos(2017.5) - etrading_ym_to_pos(2017)

mod.m <- SSModelLeadingIndicator$new(
  Y = etrading_idx,
  sea.period = 12,
  n.lag = n.lag.m,
  start = etrading_ym_to_pos(2017.5),
  end = etrading_ym_to_pos(2021 + 1 / 12),
